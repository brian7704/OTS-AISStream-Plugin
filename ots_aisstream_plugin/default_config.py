...
class DefaultConfig:
            ...
            for key, value in config.items():
                if key not in DefaultConfig.__dict__.keys():
                    ...
                    if (
                        ...
                        or value.strip() == ""
                        or not value.startswith("a-")
                        or len(value.strip()) < 5
                    ):
                        return {
                            "success": False,
                            "error": f"{key} must start with a- and be at least 5 characters",
                        }
                    elif value[2] not in "fhupansjk":
                        return {
                            "success": False,
                            "error": f"CoT type affiliation must be one of f, h, u, p, a, n, s, j, or k: {value[2]}",
                        }
    ...
    def change_config_setting(setting, value):
        try:
            ...
            logger.error(
                "Failed to change setting {} to {} in config.yml: {}".format(
                    setting, value, e
                )
            )
